...
interface CategoriesSidebarProps {
  ...
  toolCounts?: Record<Category, number>;
}

export function CategoriesSidebar({
  activeCategory,
  onCategoryChange,
  toolCounts = {} as Record<Category, number>
  ...
  return (
    <aside
      className={`hidden md:block fixed left-0 top-0 h-screen bg-card border-r border-border transition-all duration-300 ease-in-out z-40 ${
    ...
    >
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border">
          {!isCollapsed && (
            <h2 className="font-bold text-lg bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent animate-in fade-in duration-200">
              OpenKit.tools
            </h2>
          )}
          <button
            ...
            {categories.map((category) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 group relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                    isActive
                      ? "bg-zinc-600 text-white dark:bg-muted dark:text-white"
                      : "text-muted-foreground hover:text-foreground/90 hover:bg-muted/50 active:bg-muted/40"
                  }`}
                  title={isCollapsed ? category.name : category.description}
                  aria-current={isActive ? "page" : undefined}
                >
                  {/* Icon with gradient background - ensures white icons */}
                  <div
                    className={`w-8 h-8 rounded-lg bg-gradient-to-br ${category.color} text-white flex items-center justify-center shrink-0 transition-transform ${
                  ...
                  {isCollapsed && (
                    <div className="absolute left-full ml-2 px-3 py-2 bg-muted border border-border/80 rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none whitespace-nowrap z-50 shadow-xl">
                      ...
                      {/* Tooltip arrow */}
                      <div className="absolute right-full top-1/2 -translate-y-1/2 border-4 border-transparent border-r-muted" />
                    </div>
                  )}
                </button>
              );
